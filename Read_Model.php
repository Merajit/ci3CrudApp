...
class Read_Model extends CI_Model{
	...
	{
		$ret=$this->db->select('id,toDo,SubmitedDate,PostingDate')->where('id',$id)->get('tododiary');
		return $ret->row();    
	}
	...
	function insertToDo($toDo,$SubmitedDate)
	{
		...
		$sql_query=$this->db->insert('tododiary',$data);
		if($sql_query){
	...
	function updateToDo($toDo,$SubmitedDate,$toDoid)
	{
		...
		$sql_query=$this->db->where('id', $toDoid)->update('tododiary', $data); 
		if($sql_query){
	...
	{
		$sql_query=$this->db->where('id', $toDoid)->delete('tododiary'); 
		if($sql_query){
